...
cp -R /front/* /opt/FE20-MMK/react/front/
cp ROOT.war /opt/FE20-MMK/spring/
cp *.sql /opt/FE20-MMK/postgres/scripts/
cp *.yml /opt/FE20-MMK/
cp Dockerfile /opt/FE20-MMK/spring/
cp nginx.conf /opt/FE20-MMK/react/
cp /temp/* /opt/FE20-MMK/spring/temp/

#разворачиваем контейнеры
cd /opt/FE20-MMK && docker-compose up --build -d
